...
/*
"幂等" 的概念
"幂等"是指一个操作或函数，无论执行多少次，结果都是相同的。换句话说，
对于给定的输入，多次执行相同的操作或函数不会产生额外的影响或副作用。
 */
...
/*
如果我传入的a不一样，每次返回的结果都不一样，还算幂等吗？当然算，
我们说幂等的前提是保持参数一致，如果参数一致的情况下，结果永远都是a+1，因此方法是幂等的。
*/
...
有时候，我们希望某个remember变量在恰当的时候发生变化，例如int类型的变量num变化的时候，
自动生成对应的字符串，我们可以使用remember的key，当key发生变化的时候，remember的变量会重新生成。
kotlin复制代码

var num by remember { mutableStateOf(0) }
val numString = remember(key1=num) {"我是数字$num"}

上述案例中，numString是受num影响的，如果num不变的情况下，numString取的值永远都是上一次生成的值，
一旦num发生了变化，即remember中的key值变化，那么remember的lambda会重新执行来获取新值。
...
/*MutableState对象的作用就是一种可以被Compose观察其变化的对象，当一个MutableState变化的时候，
这个对象所在的所有重组作用域都会进入重组*/
...
在组合的区域启动协程 ， 因为组合区域不知道什么时候调用 ，所以用 key1 来控制
 */
@Composable
fun CoroutineTest() {
...
总结 ： 在组合情况下 LaunchedEffect ， 拿到最新的值
 */
...
/*
DisposableEffect和LaunchedEffect很类似，都有key作为重启的标识，只是必须调用onDispose方法结尾，
在onDispose中进行解绑操作。
...
有时候我们希望某个状态发生改变的时候，会改变另外一个状态的值，通常可以使用remember的key来完成这个业务，
例如下图，showText的值会在num改变的时候重新生成。
...
/*
但是有些可观察的状态我们是无法使用为remember的key的，因为变化并不是发生在它自身的值的变化，
而是其内部的值发生了变化，
...
fun ShowText5() {
    ...
    val showText = remember(list) {//没有用 因为list 没有改变
        "当前数值；${list.size}"
    }
    Column {
        Button(onClick = { list.add("") }) {//这里就是 list 内容变化了
            Text("点击我加 1")
        }
        Text(text = showText)
    }
...
/*
为什么会没用呢，因为remember比较的额是对象自身，而不是对象内部的内容，对于list来说，
它从来没有改变为其他引用，我们只改变它内部的元素，
...
总结
笔者大致讲了一下Compose的基础概念和入门难点，希望大家在入门Compose的过程中少走弯路，
多理解不同api在合适的场景下的作用，
